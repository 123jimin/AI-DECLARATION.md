...
`AI-DECLARATION.md` 파일은 구조화된 필드를 기입하기 위해 YAML 프런트매터(frontmatter)를 사용하며, 그 아래 마크다운 본문에는 사람들이 읽고 맥락을 파악할 수 있도록 필수적으로 `## Notes(참고 사항)` 섹션을 두어야 합니다. 최소한 `version`(버전), `level`(단계), 그리고 `## Notes` 섹션이 포함되어야 합니다.

선택 사항으로, 각각 개별 단계를 갖는 `processes`(과정)를 명시할 수 있습니다. 이때 전역 `level`은 명시된 단계 중 가장 높은 단계여야 합니다. 목록에 없는 과정은 암묵적으로 `none`으로 간주됩니다. 또한 개별 단계가 지정된 `components`(파일 경로 또는 디렉터리)를 나열할 수도 있습니다.

이 명세는 `version`, `level`, `processes`, `components`를 공식적으로 정의합니다.

#### 단계 (`level`)

- `none`: 어떠한 시점에서도 AI 도구를 사용하지 않음.
- `hint`: AI 자동 완성 또는 인라인 추천만 사용. 사람이 모든 코드를 작성하며, AI는 이따금 코드의 한 줄이나 블록을 완성함.
- `assist`: 사람이 주도함. 특정 작업(함수 생성, 코드 설명, 테스트 초안 작성 등)을 위해 필요할 때만 AI를 사용하지만, AI가 전반적인 작업을 이끌지는 않음.
...
- `design`: 아키텍처, 시스템 설계 및 의사 결정.
- `implementation`: 프로덕션 코드 작성.
- `testing`: 테스트 코드 작성, 테스트 계획 수립 및 품질 보증(QA).
- `documentation`: 문서, 주석, README 및 변경 로그(changelog) 작성.
- `review`: 코드 리뷰 및 풀 리퀘스트(PR) 피드백.
- `deployment`: CI/CD 구성, 인프라 및 릴리스 스크립트 작성.

### 스키마

다음 YAML 스키마는 `AI-DECLARATION.md` 파일의 구조를 공식적으로 정의합니다. 선언 파일을 검증하거나 관련 도구를 개발할 때 이 스키마를 사용하세요.
...
아래에서 다양한 상황에 따른 작성 예시를 확인할 수 있습니다.

#### 기본

가장 단순한 형태의 `AI-DECLARATION.md`에는 `version`, `level`, 그리고 `## Notes` 섹션이 필요합니다.
